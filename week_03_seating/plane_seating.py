...
def seat_economy(plane,economy_sold,name):
    ...
    #  code to seat all the economy_sold people (single)
    found_seat = False
...
def fill_plane(plane):
    ...
    # print(economy_sold)
    # print("inside fill plane()")
    for name in economy_sold.keys():
        for i in range(economy_sold[name]):
            plane = seat_economy(plane,economy_sold,name)

    ep_inside_list = []  # economy plus mid section list
    for r in plane:
        for item in r[1:-1] :
            if item[0] == 'e':
                ep_inside_list.append(item)
    # print(ep_inside_list)
    u_list = list()   # unassigned economy list in mid section
    for key, value in economy_sold.items() :
        u_list.append((value, key))
    u_list.sort(reverse = True)

    # print(u_list)
    current_row_3 = 0

    for key, value in u_list:
        if key == 3 :
            plane[current_row_3][1] = value
            plane[current_row_3][2] = value
            plane[current_row_3][3] = value
            current_row_3 = current_row_3 + 1
    current_row_2 = current_row_3  # last seating of 3 seats together increments curren_row_3 to next row
    for key, value in u_list:
        if key == 2:
            plane[current_row_2][1] = value
            plane[current_row_2][2] = value
            plane[current_row_2][3] = "avail"
            current_row_2 = current_row_2 + 1
    for row in range(len(plane)):
        if row >= current_row_2 :
            plane[row][1] = "avail"
            plane[row][2] = "avail"
            plane[row][3] = "avail"
    for key, value in u_list:           # ep_inside_list also contain unassigned single economy passengers
        if key == 1:
            ep_inside_list.append(value)
    # print(ep_inside_list)
    for single_passenger in ep_inside_list :
        plane = seat_economy(plane, economy_sold, single_passenger)
